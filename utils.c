...
char * get_time_string_realtime(struct timespec t0,struct timespec t1)
{
  const size_t MAXLINESIZE = 1024;
  char *time_string = my_malloc(sizeof(char), MAXLINESIZE);
  double timediff = t1.tv_sec - t0.tv_sec;
  ...
  if(timediff < ratios[2]) {
    my_snprintf(time_string, MAXLINESIZE,"%6.3lf secs",1e-9*(t1.tv_nsec-t0.tv_nsec) + timediff);
  }  else {
    size_t curr_index = 0;
    while (which < 4) {
      time_to_print = floor(timeleft/ratios[which]);
...
int pread_pwrite_copy(int in_fd, int out_fd, off_t in_offset, off_t out_offset, size_t nbytes, void *buf)
{
  while(nbytes > 0) {
	ssize_t bytes_read = pread(in_fd, buf, nbytes, in_offset);
	if(bytes_read < 0) {
	  fprintf(stderr,"Error in pread. Expected to read %zu bytes but read %zd bytes instead\n",nbytes, bytes_read);
	  perror(NULL);
	  return bytes_read;
	}
	ssize_t bytes_written = pwrite(out_fd, buf, nbytes, out_offset);
	if(bytes_written < 0) {
	  fprintf(stderr,"Error in pwrite. Expected to read %zu bytes but read %zd bytes instead\n",nbytes, bytes_written);
	  perror(NULL);
	  return bytes_written;
	}
	size_t bytes_transferred = (size_t) bytes_written;

	nbytes -=bytes_transferred;
	in_offset += bytes_transferred;
	out_offset += bytes_transferred;
  }
